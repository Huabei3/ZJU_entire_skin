...
%  for j=1:16
%      if a1(j,i)>0.5
%          plot(b(j,3*i-1),b(j,3*i),'+','Color',[0.7,0,0],'MarkerSize',15);
%      else
%          plot(b(j,3*i-1),b(j,3*i),'x','Color',[0,1,0],'MarkerSize',15);
%      end
%  end
%   for jj=17:48
%      if a1(jj,i)>0.5
%          plot(b(jj,3*i-1),b(jj,3*i),'+','Color',[0,0,0.8],'MarkerSize',8);
%      else
%          plot(b(jj,3*i-1),b(jj,3*i),'x','Color',[0,0.6,0.8],'MarkerSize',8);
%      end
%  end
end
axis([-40 80 -40 100]);hold on;grid on;
