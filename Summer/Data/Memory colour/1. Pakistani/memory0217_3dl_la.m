...
c(2,1:3)=max(b(:,3*i-2:3*i));
 ...
 for t=1:100
     ...
     y=(1./(1+exp(a(1)*sqrt((xdata(:,1)-a(5)).^2+a(2).*(xdata(:,2)-a(6)).^2+a(3)*(xdata(:,3)-a(7)).^2+a(4)*(xdata(:,2)-a(6)).*(xdata(:,3)-a(7)))-a(8)))).*(((xdata(:,1)-a(5)).^2+a(2).*(xdata(:,2)-a(6)).^2+a(3)*(xdata(:,3)-a(7)).^2+a(4)*(xdata(:,2)-a(6)).*(xdata(:,3)-a(7)))>=0);
     r=corr(y,ydata);
     if r >= rmax
        rmax=r;
        afinal=a;
...
%   scatter(y,ydata);
 par(i,9)=r;
f=['sqrt(',num2str(par(i,2)),'*(x-',num2str(par(i,6)),')^2+',num2str(par(i,1)),'*(y-',num2str(par(i,5)),')^2)','=',num2str(par(i,8))];
...
axis([35 65 25 55]);hold on;grid on;
