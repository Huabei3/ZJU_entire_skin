...
%   scatter(y,ydata);
 par(i,11)=r;
f=['sqrt(',num2str(par(i,2)),'*(x-',num2str(par(i,6)),')^2+',num2str(par(i,1)),'*(y-',num2str(par(i,5)),')^2+',num2str(par(i,9)),'*(x-',num2str(par(i,6)),')','*(y-',num2str(par(i,5)),'))','=',num2str(par(i,8))];
h=ezplot(f,[-100,120]);set(h,'Color',[0.5,0,0]);hold on;
text(par(i,6)+1,par(i,5),num2str(i),'fontsize',10);
plot(par(i,6),par(i,5),'o','Color',[0.5,0,0],'MarkerFaceColor',[0.5,0,0],'MarkerSize',3);hold on;
for j=17:32
     if a3(j,i)>0.5
