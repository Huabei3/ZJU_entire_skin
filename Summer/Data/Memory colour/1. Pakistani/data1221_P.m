...
h=ezplot(f,[-100,120]);set(h,'Color',[0,0,0.5]);hold on;
plot(par(i,6),par(i,7),'o','Color',[0,0,0.5],'MarkerFaceColor',[0,0,0.5],'MarkerSize',3);hold on;
 plot([par(i,6),par1(i,6)],[par(i,7),par1(i,7)],'Color',[0.3 0.3 0.3]);
%  text(par(i,6),par(i,7)-5,num2str(i));
%  for j=1:16
%      if a1(j,i)>0.5
% %          plot(b(j,3*i-1),b(j,3*i),'+','Color',[1,0,0],'MarkerSize',8);
%            text(b(j,3*i-1),b(j,3*i),strcat(num2str(round(a1(j,i)*100)),'%'),'Color',[1,0,0]);
%      else
% %          plot(b(j,3*i-1),b(j,3*i),'x','Color',[0,1,0],'MarkerSize',8);
%            text(b(j,3*i-1),b(j,3*i),strcat(num2str(round(a1(j,i)*100)),'%'),'Color',[0,1,0]);
%      end
%  end
 end
% % 
