...
%   scatter(y,ydata);
 par(i,11)=r;
f=['sqrt(',num2str(par(i,3)),'*(x-',num2str(par(i,7)),')^2+',num2str(par(i,1)),'*(y-',num2str(par(i,5)),')^2+',num2str(par(i,10)),'*(x-',num2str(par(i,7)),')','*(y-',num2str(par(i,5)),'))','=',num2str(par(i,8))];
h=ezplot(f,[-100,120]);set(h,'Color',[0.5,0,0]);hold on;
text(par(i,7)+1,par(i,5),num2str(i),'fontsize',10);
plot(par(i,7),par(i,5),'o','Color',[0.5,0,0],'MarkerFaceColor',[0.5,0,0],'MarkerSize',3);hold on;
for j=33:48
     if a3(j,i)>0.5
...
axis([25 55 25 55]);hold on;grid on;
