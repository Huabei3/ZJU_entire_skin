...
load('plot3d_0106.mat');
%===========exemple delta LCh===================================================
for i=1:24;
[x,y,z]=meshgrid(linspace(-100,100,401));
val=par(i,1).*z.^2+par(i,2).*x.^2+par(i,3).*y.^2+par(i,4).*x.*y++par(i,5).*x.*z++par(i,6).*z.*y;
[f,v]=isosurface(x,y,z,val,par(i,7).^2);
subplot(4,6,i);
v2(:,1)=v(:,1);v2(:,2)=v(:,3);v2(:,3)=v(:,2);
patch('Faces',f,'Vertices',v2,'facecolor','w','edgecolor','flat','CData',v2(:,3))
grid on;view(3);
axis([-20 20 -20 20 -20 20]);axis equal;grid on;hold on;
xlabel('delta C*ab','FontSize',5);
ylabel('delta hab*','FontSize',5);
zlabel('delta L*','FontSize',5);
set(gca,'XTick',[-20:10:20]);%设置要显示坐标刻度
set(gca,'YTick',[-20:10:20]);%设置要显示坐标刻度
set(gca,'ZTick',[-20:10:20]);%设置要显示坐标刻度
plot([0,0],[-20,20],'Color',[1,0.1,0.1]);
plot([-20,20],[0,0],'Color',[1,0.1,0.1]);
plot3([0,0],[0 0],[-20,20],'Color',[1,0.1,0.1]);
clear v;clear v2;
end
%===========exemple LCh 2 lab===================================================
% subplot(1,3,2);
