...
h=ezplot(f,[-100,120]);set(h,'Color',[0.2,0.2,0.2],'LineWidth',3);hold on;
...
plot(par(i,6),par(i,7),'o','Color',[0.2,0.2,0.2],'MarkerFaceColor',[0.2,0.2,0.2],'MarkerSize',6);hold on;
 for j=17:49
     if an(j,i)>0.5
         plot(b(j,3*i-1),b(j,3*i),'+','Color',[0.8,0.1,0],'MarkerSize',12,'LineWidth',2);
     else
         plot(b(j,3*i-1),b(j,3*i),'x','Color',[0.2,0.7,0.2],'MarkerSize',12,'LineWidth',2);
     end
 end
 %==========================plot============================================
title(strcat('Memory colour ',num2str(i),' on CIELAB C*_a_bL*'));
axis([40 80 20 60]);hold on;grid on;
