...
h=ezplot(f,[-100,120]);set(h,'Color',[0.1,0.5,0.1]);hold on;
text(par(i,6)+1,par(i,7),num2str(i),'fontsize',10);
plot(par(i,6),par(i,7),'o','Color',[0.1,0.5,0.1],'MarkerFaceColor',[0.1,0.5,0.1],'MarkerSize',6);hold on;
par1=par;
%===================Preference=================
  load('Leeds_all_1226.mat');
  for j=1:28  
...
ydata=ap(:,i);
 ...
 par(i,9)=r(1,2);
f=['sqrt(',num2str(par(i,2)),'*(x-',num2str(par(i,6)),')^2+',num2str(par(i,3)),'*(y-',num2str(par(i,7)),')^2+',num2str(par(i,4)),'*(x-',num2str(par(i,6)),')','*(y-',num2str(par(i,7)),'))','=',num2str(par(i,8))];
h=ezplot(f,[-100,120]);set(h,'Color',[0.5,0.1,0.1]);hold on;
plot(par(i,6),par(i,7),'o','Color',[0.5,0.1,0.1],'MarkerFaceColor',[0.5,0.1,0.1],'MarkerSize',6);hold on;
plot([par(i,6),par1(i,6)],[par(i,7),par1(i,7)],'Color',[0.5,0.5,0.5]);
end
