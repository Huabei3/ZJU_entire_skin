...
%  par(i,9:10)=par(i,9:10).*3;
 ...
 r=corr(y,ydata);
 par(jj,11)=r;
f=['sqrt(',num2str(par(jj,2)),'*(x-',num2str(par(jj,6)),')^2+',num2str(par(jj,3)),'*(y-',num2str(par(jj,7)),')^2+',num2str(par(jj,4)),'*(x-',num2str(par(jj,6)),')','*(y-',num2str(par(jj,7)),'))','=',num2str(par(jj,8))];
h=ezplot(f,[-100,120]);set(h,'Color',[0.2,0.2,0.2]);hold on;
text(par(jj,6)+1,par(jj,7),num2str(jj),'fontsize',10);
plot(par(jj,6),par(jj,7),'o','Color',[0.2,0.2,0.2],'MarkerFaceColor',[0.2,0.2,0.2],'MarkerSize',3);hold on;
end
end
