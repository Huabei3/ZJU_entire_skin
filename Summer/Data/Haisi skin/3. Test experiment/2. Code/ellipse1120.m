...
for ob=1:28
    ...
    b=xyz2lab(f(:,3*ol-2:3*ol),'user',f1(ol,:)./f1(ol,2).*475); 
    b(:,3)=b(:,3)+2;
...
[h,V,ab1(il,1),ab1(il,2)] = ellipsefig0621(a(i,2:3),s,r,1,il); % 绘制置信椭圆
...
alpha=0.05;                 %显著性水平，标量，取值介于[0,1]，默认值为0.05
r = chi2inv(1-alpha,p);     %基于正态分布
[h,V,ab1(il,1),ab1(il,2)] = ellipsefig0621(a(i+5,2:3),s,r,1,il); % 绘制置信椭圆
ab1(il,3)=atan(V(1,1)./V(1,2))./pi.*180;
axis([-20 40 -20 40]);hold on;grid on;
 set(gca,'XTick',[-20:10:40]);%设置要显示坐标刻度
 set(gca,'YTick',[-20:10:40]);%设置要显示坐标刻度
...
alpha=0.05;                 %显著性水平，标量，取值介于[0,1]，默认值为0.05
r = chi2inv(1-alpha,p);     %基于正态分布
[h,V,ab1(il,1),ab1(il,2)] = ellipsefig0621(a(i+10,2:3),s,r,1,il); % 绘制置信椭圆
ab1(il,3)=atan(V(1,1)./V(1,2))./pi.*180;
axis([-20 40 -20 40]);hold on;grid on;
 set(gca,'XTick',[-20:10:40]);%设置要显示坐标刻度
 set(gca,'YTick',[-20:10:40]);%设置要显示坐标刻度
...
alpha=0.05;                 %显著性水平，标量，取值介于[0,1]，默认值为0.05
r = chi2inv(1-alpha,p);     %基于正态分布
[h,V,ab1(il,1),ab1(il,2)] = ellipsefig0621(a(i+15,2:3),s,r,1,il); % 绘制置信椭圆
ab1(il,3)=atan(V(1,1)./V(1,2))./pi.*180;
axis([-20 40 -20 40]);hold on;grid on;
 set(gca,'XTick',[-20:10:40]);%设置要显示坐标刻度
 set(gca,'YTick',[-20:10:40]);%设置要显示坐标刻度
...
sgtitle('Preferred skin color centers on CIELAB (gray background)');