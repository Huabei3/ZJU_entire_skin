function par = ellipsoidfit6(cielab,zscore,colorcenter)
...
if nargin < 3
    ...
    for t = 1:100
        ...
        a = lsqcurvefit(f,a0,xdata,ydata,[0,0,-1,1,10,10],[0.2,1,0,4,21,35],options);
...
%     fprintf('corralation coefficient is %.2f\n',r);
    ...
    for t = 1:100
        a0 = [-rand,rand,rand,rand,rand,rand];
        options = optimset('MaxFunEvals',200000);
        a = lsqcurvefit(f,a0,xdata,ydata,[-inf,0,-inf,-inf,10,10],[inf,inf,inf,inf,21,35],options);
    ...
    par(5:7) = colorcenter;
    a = afinal;
    y = (1./(1+exp(a(1)*sqrt((xdata(:,2)-colorcenter(2)).^2+a(2)*(xdata(:,3)-colorcenter(3)).^2+a(3)*(xdata(:,2)-colorcenter(2)).*(xdata(:,3)-colorcenter(3)))-a(4)))).*(((xdata(:,2)-colorcenter(2)).^2+a(2)*(xdata(:,3)-colorcenter(3)).^2+a(3)*(xdata(:,2)-colorcenter(2)).*(xdata(:,3)-colorcenter(3)))>=0);     scatter(y,ydata);
    r = corr(y,ydata);
    par(7)=r;
%     fprintf('corralation coefficient is %.2f\n',r);
end
