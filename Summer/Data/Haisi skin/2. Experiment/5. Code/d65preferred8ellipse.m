...
color=[255,185,0;0,205,0;255,29,0;15,74,155;15,74,155;255,29,0;255,185,0;0,205,0]./255;
lin=[".","-","-",".","-",".","-","."];
for j=1:8
 score1=f1(:,(3*j-2):(3*j));
[m,n] = size(score1);
...
h = plot(xy(1,:)+M(2),xy(2,:)+M(3),lin(j), 'color',color(j,:),'linewidth', 2);
plot(M(2),M(3),'o','MarkerEdgeColor',color(j,:),'MarkerFaceColor',color(j,:),'MarkerSize',5);
...
plot(M(2),M(3),'^','MarkerEdgeColor',color(j,:),'MarkerFaceColor',color(j,:),'MarkerSize',5);
...
xy = V*[aa*cos(t);bb*sin(t)];   % 坐标旋转
h = plot(xy(1,:)+M(2),xy(2,:)+M(3),'.', 'color',color(j,:),'linewidth', 1.5);
plot(M(2),M(3),'s','MarkerEdgeColor',color(j,:),'MarkerFaceColor',color(j,:),'MarkerSize',5);

axis([0 30 0 30]);hold on
title('Caucasian Preferrred skin color','FontSize',20)
