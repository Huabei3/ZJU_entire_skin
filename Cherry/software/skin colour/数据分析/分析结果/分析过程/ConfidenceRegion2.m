function [V,xm,aa,bb,cc] = ConfidenceRegion2(xdat,alpha,distribution)
%    绘制置信区域（区间、椭圆区域或椭球区域）
%    应用场景：已知点的分布，并假设这些点在某个椭圆的内部，求该外包椭圆
...
switch p
     ...
     case 2     % 二维情形（置信椭圆）
         x = xdat(:,1);
         y = xdat(:,2);
         s = inv(cov(xdat));   % 协方差矩阵 求逆矩阵是为了下面的基于正态分布
         %s = cov(xdat);   %测试
     ...
     case 3     % 三维情形（置信椭球）
         ...
         if strncmpi(distribution,'norm',3)
             r = chi2inv(1-alpha,p);
             %r = p*(n-1)*finv(1-alpha,p,n-p)/(n-p)/n;
             TitleText = 'Observer:All - Image:Caucasian';
         else
             r = prctile(rd,100*(1-alpha));
...
%           plot3(x(rd>r),y(rd>r),z(rd>r),'r+','MarkerSize',10)   % 画样本散点
         ...
         hold on
         [h,V,aa,bb,cc] = ellipsefig(xm,s,r,2);   % 绘制置信椭球
         xlabel('R')
         ylabel('G')
         zlabel('B')
         title(TitleText)
...
if tag == 1
     ...
     h = plot(xy(1,:)+xc(1),xy(2,:)+xc(2), 'b', 'linewidth', 2);
...
%      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     h = mesh(x,y,z,'EdgeColor',[0.5 0.5 0.5]);   % 绘制椭球面网格图
end
