load all-S
load("..\TargetLab_Chinese3.mat");
load("compare_new.mat");
...
Lab(:,2)=score(:,3);
Lab(:,3)=score(:,1);
[V,xm,aa,bb,cc] = ConfidenceRegion2(Lab,0.05,'norm');
axis equal;
hold on;
xlabel('R','FontSize',15);
ylabel('G','FontSize',15);
zlabel('B','FontSize',15);
axis([10 30 10 30 30 70]);
...
LAB2(:,2)=lab0_new(:,3);
LAB2(:,3)=lab0_new(:,1);
[V,xm,aa,bb,cc]=ConfidenceRegion2(LAB2,0.05,'norm');
axis equal;
hold on;
axis([0 25 0 25 30 80]);
grid on
plot3(mlab(2),mlab(3),30,'b-o'); 
plot3(mlab_new(2),mlab_new(3),30,'r-o'); 
plot3(yc,zc,30,'k-o'); 
    hold on;