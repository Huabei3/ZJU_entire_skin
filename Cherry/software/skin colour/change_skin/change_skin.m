[~,~,idx1]=imread('YY_F.png');
idx1(idx1>0)=1;
[~,~,idx2]=imread('YY_F_MASK.png');
idx2(idx2>0)=1;
idx=idx1-idx2;                               %%得到index

[~,~,idx3]=imread('YY_F_Skin.png');
idx3(idx3>0)=1;
idx0=idx1-idx3;  

img=double(imread('YY_F.png'));
height=size(img,1);
weight=size(img,2);
...
for i=1:49
    ...
    imwrite(rimg,strcat('YY_F',num2str(i),'.png'),'png');
end
