function XYZ = CAM16UCS2XYZ(JAB,XYZw,La,Yb,surround,CCT,Duv)
%%% CAM16-UCS developed by MUHAMMAD SAFDAR on 2017.03.14, Email: msafdar87@hotmail.com %%%

JCh = Jab2JCh(JAB, La);
XYZ= CAM16_rev(JCh,XYZw,La,Yb,surround,CCT,Duv);
end

function XYZ = CAM16_rev(JCh,XYZw,La,Yb,Surround,CCT,Duv)
%%% XYZ is test XYZ [Nx3]
...
D_pre = 0.723*(1-1116/CCT+8.46*Duv-49266*Duv/CCT);
if D_pre<0
    D = 0;
elseif D_pre>1
    D = 1;
...
function JCh = Jab2JCh(JAB, La)
%%% JCh is a [Nx3] vector
%%% JAB is a [Nx3] vector

Jp = JAB(:,1); ap = JAB(:,2); bp =  JAB(:,3);

c1 = 0.007; c2 = 0.0228; % Uniform Colour Spaces Based on CIECAM02 Colour Appearance Model by Luo et al.2002

J = Jp./(1+100*c1-c1*Jp); 
for i=1:size(JAB,1)
h(i) = car2pol([ap(i),bp(i)]);
end
h = (h<0).*(h+360) +(1-(h<0)).*h;
h=h';
        
M = (1/c2).*(exp((c2.*ap)./(cos(h.*pi./180)))-1);
