function index = natural_test_skin2(imgname)
...
%     dc = abs(dc);
    ...
    index = find((dl >= -30) &(dl <= 30) & (dh <= 30) & (dc <= 30)& (dc >= -30));
    index = index';
    skin2 = length(index) / (sz(1) * sz(2));
    if ~isempty(index)
        for k = 1 : size(index')
            lab1(index(k),:) = [0 0 0];
        end
