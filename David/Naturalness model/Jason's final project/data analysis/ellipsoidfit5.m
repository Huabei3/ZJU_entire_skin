function [par,r] = ellipsoidfit5(cielab,zscore,colorcenter)
...
% output: par = [k1,k2,k3,alpha,a0,b0]
if nargin < 3
    xdata = cielab;
    ydata = zscore;
    ...
    for t = 1:100
        ...
        [a,res] = lsqcurvefit(f,a0,xdata,ydata,[],[],options);
...
%     fprintf('rmin is %.2f\n',rmin);
    ...
    f = @(a,xdata)(1./(1+exp(a(1)*sqrt((xdata(:,2)-colorcenter(2)).^2+a(2)*(xdata(:,3)-colorcenter(3)).^2+a(3)*(xdata(:,2)-colorcenter(2)).*(xdata(:,3)-colorcenter(3)))-a(4)))).*(((xdata(:,2)-a(5)).^2+a(2)*(xdata(:,3)-a(6)).^2+a(3)*(xdata(:,2)-a(5)).*(xdata(:,3)-a(6)))>=0);
    rmax = 0;
    ...
    for t = 1:100
        ...
        y = (1./(1+exp(a(1)*sqrt((xdata(:,2)-colorcenter(2)).^2+a(2)*(xdata(:,3)-colorcenter(3)).^2+a(3)*(xdata(:,2)-colorcenter(2)).*(xdata(:,3)-colorcenter(3)))-a(4)))).*(((xdata(:,2)-a(5)).^2+a(2)*(xdata(:,3)-a(6)).^2+a(3)*(xdata(:,2)-a(5)).*(xdata(:,3)-a(6)))>=0);
        r = corr(y,ydata);
        if r >= rmax
            rmax = r;
            afinal = a;
